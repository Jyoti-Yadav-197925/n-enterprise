...
// Interest Rate Compare Component - KEEPING YOUR EXACT CODE
const InterestRateCompare = () => {
  const banks = [
    { name: 'SBI', home: '8.40%', business: '9.75%', personal: '10.50%', property: '8.65%', logo: '🏦' },
    { name: 'HDFC', home: '8.50%', business: '10.00%', personal: '10.75%', property: '8.75%', logo: '🏛️' },
...
// Document Checklist Component - KEEPING YOUR EXACT UI, ADDING IDs FOR REDIRECTS
const DocumentChecklist = () => {
  const [activeTab, setActiveTab] = useState('salaried')

  // Listen for hash changes to auto-switch tab when redirected from navbar
  useEffect(() => {
...
// Loan Eligibility Component - KEEPING YOUR EXACT CODE
const LoanEligibility = () => {
  const [monthlyIncome, setMonthlyIncome] = useState(50000)
  const [existingEMI, setExistingEMI] = useState(0)
  const [loanType, setLoanType] = useState('home')
...
export default function ToolsSection() {
  ...
  useEffect(() => {
    const hash = window.location.hash
    if (hash === '#salaried-docs' || hash === '#business-docs' || hash === '#property-docs') {
      setActiveTool('documents')
    } else if (hash === '#emi-calculator') {
      setActiveTool('emi')
    } else if (hash === '#rate-compare') {
      setActiveTool('rates')
    } else if (hash === '#loan-eligibility') {
      setActiveTool('eligibility')
    }
  }, [])
